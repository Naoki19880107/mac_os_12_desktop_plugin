...
class Lunar(object):
    ...
    def ln_jie(self):  # 返回农历节气
        ...
        for i in range(24):
            # 因为两个都是浮点数，不能用相等表示
            delta = self._julian_day() - Lunar._julian_day_of_ln_jie(year, i)
            if -.5 <= delta <= .5:
    ...
    def _julian_day_of_ln_jie(year, st):
        s_stAccInfo = [
    ...
    def getLunar(ct=None):
        ln = Lunar(ct)
        if ln.ln_jie():
            return '农历 {}年 {} {}'.format(ln.gz_year(), ln.ln_date_str(), ln.ln_jie())
        else:
            return '农历 {}年 {}'.format(ln.gz_year(), ln.ln_date_str())
